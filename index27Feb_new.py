...
# For cross-validation and other scenarios where we need the original dataset
...
# Implement stratified split to ensure class balance
...
class ActivityClassifier(nn.Module):
    def __init__(self, feature_dim=512, num_classes=11):
        ...
        self.dropout1 = nn.Dropout(0.7)  # Increased dropout
        self.fc2 = nn.Linear(256, 128)
        self.bn2 = nn.BatchNorm1d(128)   # Additional layer
        self.dropout2 = nn.Dropout(0.5)  # Additional dropout
...
def perform_cross_frequency_analysis():
    """Evaluate cross-frequency transfer learning performance."""
    # Define radar types
    radar_types = ['24GHz', '77GHz', 'Xethru']
    
    # Create separate datasets for each radar type
    radar_datasets = {}
    radar_loaders = {}
    
    for radar_type in radar_types:
        # Create dataset with only this radar type
        ...
        single_radar_dataset = RadarSpectrogramDataset(
            root_dir=r"C:\Users\mk305r\Desktop\Multi-StreamRadar\11_class_activity_data", 
            transform=transform,
            radars=[radar_type]  # Only use this specific radar
        )
        
        # Split into train/test
        indices = list(range(len(single_radar_dataset)))
        labels = [single_radar_dataset.samples[i][1] for i in indices]
        train_indices, test_indices = train_test_split(
            indices, test_size=0.2, stratify=labels, random_state=42
        )
        
        train_dataset = torch.utils.data.Subset(single_radar_dataset, train_indices)
        test_dataset = torch.utils.data.Subset(single_radar_dataset, test_indices)
        
        # Create data loaders
        radar_datasets[radar_type] = {
            'train': train_dataset,
            'test': test_dataset
        }
        
        radar_loaders[radar_type] = {
            'train': DataLoader(train_dataset, batch_size=16, shuffle=True, num_workers=0),
            'test': DataLoader(test_dataset, batch_size=16, shuffle=False, num_workers=0)
        }
    
    # Results table
    cross_frequency_results = {}
    
    # For each radar type, train a model and test on other radars
    for train_radar in radar_types:
        cross_frequency_results[train_radar] = {}
        
        # Create a simple CNN model for this radar type
        model = RadarEncoder(pretrained=True)
        # Add classification head
        model.classifier = nn.Linear(model.feature_dim, 11)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = model.to(device)
        ...
        for epoch in range(15):  # Train for 15 epochs
            running_loss = 0.0
            for inputs, labels in radar_loaders[train_radar]['train']:
                inputs = inputs[0].to(device)  # Only one radar type
                labels = labels.to(device)
                ...
                outputs = model.classifier(features.view(-1, model.feature_dim))
                loss = criterion(outputs, labels)
            ...
            print(f"Epoch {epoch+1}, Radar: {train_radar}, Loss: {running_loss/len(radar_loaders[train_radar]['train']):.4f}")
        
        # Test on all radar types
        model.eval()
        for test_radar in radar_types:
            correct = 0
            total = 0
            
            with torch.no_grad():
                for inputs, labels in radar_loaders[test_radar]['test']:
                    inputs = inputs[0].to(device)  # Only one radar type
                    labels = labels.to(device)
            ...
            cross_frequency_results[train_radar][test_radar] = accuracy
            print(f"Train: {train_radar}, Test: {test_radar}, Accuracy: {accuracy:.2f}%")
    
    # Print results table
    print("\nCross-Frequency Transfer Learning Results:")
    print("Fine-Tuning | Testing | Accuracy (%)")
    print("-----------|---------|------------")
    
    for train_radar in radar_types:
        for test_radar in radar_types:
            print(f"{train_radar} | {test_radar} | {cross_frequency_results[train_radar][test_radar]:.2f}")
    
    return cross_frequency_results

def create_comparison_figure(cross_frequency_results):
    """Create a visualization comparing cross-frequency results with fusion approach."""
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    same_freq_accs = [cross_frequency_results[r][r] for r in radar_types]
    cross_freq_accs = []
    for train_r in radar_types:
        for test_r in radar_types:
            if train_r != test_r:
                cross_freq_accs.append(cross_frequency_results[train_r][test_r])
    
    # Calculate average accuracies
    avg_same_freq = np.mean(same_freq_accs)
    avg_cross_freq = np.mean(cross_freq_accs)
    fusion_acc = 100.0  # Your fusion approach achieves 100%
    
    # Create bar chart
    plt.figure(figsize=(10, 6))
    methods = ['Same Frequency\n(Single Radar)', 'Cross Frequency\n(Transfer Learning)', 'Our Fusion\nApproach']
    accuracies = [avg_same_freq, avg_cross_freq, fusion_acc]
    
    bars = plt.bar(methods, accuracies, color=['lightblue', 'lightcoral', 'lightgreen'])
    ...
    plt.ylim(0, 105)  # Ensure all bars are visible
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    
    # Save figure
    plt.tight_layout()
    plt.savefig('cross_frequency_comparison.png', dpi=300)
    plt.close()
    
    print("Comparison figure created and saved as 'cross_frequency_comparison.png'")
...
if __name__ == '__main__':
    ...
    print("Performing cross-frequency transfer learning analysis...")
    cross_frequency_results = perform_cross_frequency_analysis()
    
    # Optionally, add code to save or visualize these results
    print("\nCross-Frequency Transfer Learning Results:")
    print("Fine-Tuning | Testing | Accuracy (%)")
    print("-----------|---------|------------")
    ...
    create_comparison_figure(cross_frequency_results)
